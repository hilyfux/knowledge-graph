...
Glob `**/CLAUDE.md`（排除 .git、node_modules），对每个文件读取 `# 标题`。
生成 `.knowledge-graph/knowledge-index.md`，指针索引格式（模块 CLAUDE.md 由 Claude 原生子目录遍历懒加载，此处只存路径+标签）：

```
# KG Index ({ISO日期})
{路径}: {≤15字标签，如"认证/XSS"或"API/CORS"}
{路径}: {≤15字标签}
```

不用表格，一行一模块。标签只写关键词帮助定位，不写完整规则（完整规则在模块 CLAUDE.md 中，按需加载）。
...
```
此锁文件防止 hook 在更新进行中重复触发。步骤 5 收尾时删除。
</step>
...
**P6-P8 推理阶段（并行分派）**
当 event_count ≥ 15 时，使用 Agent tool 并行分派两个子代理，保护主会话上下文：

**Agent A — 依赖发现**（P6 + P7）：
用 Agent tool 启动，prompt 包含：
- 项目路径：`$CLAUDE_PROJECT_DIR`
- 指令：运行 `bash "${CLAUDE_SKILL_DIR}/scripts/infer.sh" sequences` 和 `bash "${CLAUDE_SKILL_DIR}/scripts/infer.sh" cochange`
- P6：发现 read→write 重复模式（count ≥ 2）→ 在 write_dir 的 CLAUDE.md `## 改动时` 段追加 `- 改动前 → 先看 @{read_dir}/CLAUDE.md`，已存在的引用跳过
- P7：发现 co-change 频率 ≥ 3 的目录对 → 在两个目录的 CLAUDE.md `## 改动时` 段互相引用，已存在的引用跳过
- 返回：修改了哪些文件、新增了哪些引用

**Agent B — 知识衰减**（P8）：
用 Agent tool 启动，prompt 包含：
- 项目路径：`$CLAUDE_PROJECT_DIR`
- 指令：运行 `bash "${CLAUDE_SKILL_DIR}/scripts/infer.sh" decay`
- status=stale（30+ 天无事件）→ 在该 CLAUDE.md 顶部加注释 `<!-- stale: 30+天无活动，待验证 -->`
- status=ineffective（有禁忌但仍在失败）→ 重新读取目录关键文件，用 Edit 重写禁忌使其更具体
- status=effective → 不动
- 返回：处理了哪些模块、状态变更摘要

两个 Agent 必须在同一个消息中并行发出。等待两者完成后进入收尾。
当 event_count < 15 时，跳过 P6-P8（轻量模式不运行推理）。
</step>
...
5. 删除锁文件：`rm -f "$CLAUDE_PROJECT_DIR/.knowledge-graph/.kg-updating"`
6. 输出：「更新完成：新增 {N} 个模块 / 修复 {N} 个节点 / 新增 {N} 条规则 / 发现 {N} 条隐含依赖 / {N} 个衰减节点」
</step>
