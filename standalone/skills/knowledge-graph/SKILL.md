...
<data_paths>
  cache:     .claude/skills/knowledge-graph/data/graph-analysis.json
  events:    .claude/skills/knowledge-graph/data/graph-events.jsonl
...
<step id="2" name="确认">
读取 `.claude/skills/knowledge-graph/data/graph-scan.json`，输出：
...
```bash
mkdir -p "$CLAUDE_PROJECT_DIR/.claude/skills/knowledge-graph/data"
touch "$CLAUDE_PROJECT_DIR/.claude/skills/knowledge-graph/data/graph-events.jsonl"
```
删除临时文件 `.claude/skills/knowledge-graph/data/graph-scan.json`。
</step>

<step id="6" name="生成知识索引">
Glob `**/CLAUDE.md`（排除 .git、node_modules），对每个文件读取第一行 `# 标题` 和 `## 禁忌` 下第一条。
生成 `.claude/skills/knowledge-graph/data/knowledge-index.md`：

```
# 知识索引
> 自动生成，勿手动编辑。最后更新：{ISO日期}

| 模块 | 路径 | 摘要 |
|------|------|------|
| {标题} | {相对路径}/CLAUDE.md | {禁忌第一条或约定第一条} |
```

此文件供 Claude 在对话开始时快速定位知识节点，避免 Glob 扫描。
</step>
...
生成 CLAUDE.md（≤30 行），写入前自检：每条禁忌必须有代码分析或 git 历史的具体证据。
无证据 → 不写。每条 @ 引用的目标文件必须存在，不存在 → 不写。
</step>

<step id="4" name="基于事件更新现有节点">
检查 `.claude/skills/knowledge-graph/data/graph-events.jsonl` 行数：
...
```
然后读取 `.claude/skills/knowledge-graph/data/graph-analysis.json`。
...
<step id="5" name="收尾">
1. 清空 `.claude/skills/knowledge-graph/data/graph-events.jsonl`（已分析完毕）
2. 删除 `.claude/skills/knowledge-graph/data/graph-analysis.json`（临时缓存）
3. 重新生成 `.claude/skills/knowledge-graph/data/knowledge-index.md`（同 init 步骤 6 的格式）
4. 输出：「更新完成：新增 {N} 个模块 / 修复 {N} 个节点 / 新增 {N} 条规则」
</step>
...
<step id="1" name="定位">
读取 `.claude/skills/knowledge-graph/data/knowledge-index.md`。
