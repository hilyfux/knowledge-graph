...
if echo "$PROMPT" | grep -qiE '(整理(一下|完了?)?|清理(一下|完了?)?|确认(一下|完了?)?|ok了|可以了|搞定了?|行了|没问题了|done了?|完成了)'; then
  ...
  INJECT="【kg 自动指令】检测到阶段性完成信号（${PENDING}条待分析）。请在响应用户后调用 Skill tool（skill: knowledge-graph, args: update）将本轮成果固化到知识图谱。"
fi

# 失败信号（遇到问题，记录教训）
if [ -z "$INJECT" ] && echo "$PROMPT" | grep -qiE '(还不行|还是不行|不对(啊|呀)?|又(错|报错|失败)了|还是错|仍然不行|没解决)'; then
  INJECT="【kg 自动指令】检测到失败信号。请在响应用户后调用 Skill tool（skill: knowledge-graph, args: update）将本轮失败教训记录到知识图谱禁忌。"
fi
