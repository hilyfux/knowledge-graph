...
# Critical: AI just lost all context. Inject working summary to resume.
# Budget: 5s timeout. Must be fast.
...
if [ -f "$EVENTS" ] && [ -s "$EVENTS" ]; then
  # Single jq call: extract active dirs + failures from recent events
  ...
  # Get active dir list for CLAUDE.md matching (one extraction, no per-file jq)
  ACTIVE_DIRS=$(tail -200 "$EVENTS" | jq -r 'select(.e | startswith("w")) | .p | split("/") | if length > 1 then .[:-1] | join("/") else "." end' 2>/dev/null | sort -u)
fi

# Inject prohibitions only for active directories (no per-file jq)
